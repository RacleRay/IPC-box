...
void run_sync_server(void* shm_addr, struct sync* sync, int msg_size, int msg_count);
...
int main(int argc, char *argv[]) {
    ...
    if (shm_id < 0) {
        err_sys("shm server shm get");
    }

    void* shm_addr = shmat(shm_id, NULL, 0);
    if (shm_addr == (char*)-1) {
        err_sys("shm server shmat");
    }
    ...
    // run
    run_sync_server(shm_addr, sync, args.msg_size, args.msg_count);
...
void run_sync_server(void* shm_addr, struct sync* sync, int msg_size, int msg_count) {
    ...
    printf("sync shm server start.\n");

    for (int i = 0; i < msg_count; i++) {
        // send message
        memset(shm_addr, 'c', msg_size);
